...
Claude Code 插件需要能在 Windows、macOS 和 Linux 上工作的钩子。本文档解释使这成为可能的多语言包装器技术。
...
1. **脚本执行**：Windows CMD 无法直接执行 `.sh` 文件——它尝试在文本编辑器中打开它们
...
多语言脚本在多种语言中同时有效。我们的包装器在 CMD 和 bash 中都有效：
...
您的实际钩子逻辑放在 `.sh` 文件中。为确保它在 Windows 上工作（通过 Git Bash）：
...
- 如果必须使用它们，它们在 Git Bash 中可用，但确保 PATH 已设置（使用 `bash -l`）
...
对于有多个钩子的插件，您可以创建一个通用包装器，将脚本名称作为参数：
...
### 脚本在文本编辑器中打开而不是运行
hooks.json 直接指向 `.sh` 文件。请改为指向 `.cmd` 包装器。
