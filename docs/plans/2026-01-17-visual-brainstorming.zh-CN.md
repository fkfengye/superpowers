...
> **对于代理工作者：** 必需的子技能：使用 superpowers:executing-plans 按任务逐步实现此计划。

**目标：** 为 Claude 提供一个基于浏览器的视觉头脑风暴伴侣——在终端对话旁边显示模拟、原型和交互式选择。

**架构：** Claude 将 HTML 写入临时文件。本地 Node.js 服务器监视该文件并使用自动注入的辅助库提供服务。用户交互通过 WebSocket 流向服务器 stdout，Claude 在后台任务输出中看到。
...
在 `skills/brainstorming/SKILL.md` 中的"关键原则"之后添加：
...
1. **服务器** at `lib/brainstorm-server/` - 监视 HTML 文件并中继事件的 Node.js 服务器
2. **辅助库** 自动注入 - 捕获点击、表单、输入
3. **测试** at `tests/brainstorm-server/` - 验证服务器行为
4. **头脑风暴技能** 更新了视觉伴侣部分和 `visual-companion.md` 参考文档

**使用方法：**
1. 启动服务器作为后台作业：`node lib/brainstorm-server/index.js &`
