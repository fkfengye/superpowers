...
本文档描述了如何测试 Superpowers 技能，特别是像 `subagent-driven-development` 这样的复杂技能的集成测试。

## 概述

测试涉及子代理、工作流程和复杂交互的技能需要在 headless 模式下运行实际的 Claude Code 会话，并通过会话记录验证其行为。
...
集成测试验证 `subagent-driven-development` 技能是否正确执行：

1. **计划加载**：在开始时读取计划一次
2. **完整任务文本**：向子代理提供完整的任务描述（不让他们读取文件）
3. **自我审查**：确保子代理在报告之前执行自我审查
4. **审查顺序**：在代码质量审查之前先进行规范合规性审查
5. **审查循环**：发现问题时使用审查循环
6. **独立验证**：规范审查者独立阅读代码，不信任实施者报告

### 工作原理

1. **设置**：使用最小实现计划创建临时 Node.js 项目
2. **执行**：使用技能在 headless 模式下运行 Claude Code
3. **验证**：解析会话记录（`.jsonl` 文件）以验证：
   ...
   - 测试通过
   - Git 提交显示正确的工作流程
4. **Token 分析**：显示按子代理划分的 token 使用情况
...
会话记录存储在 `~/.claude/projects/` 中，路径编码了工作目录：
...
- **高缓存读取**：好——意味着提示缓存正在工作
- **主会话输入 token 高**：预期——协调者有完整上下文
- **每个子代理成本相似**：预期——每个任务复杂度相似
- **每个任务成本**：典型范围是每个子代理 $0.05-$0.15，取决于任务
...
5. **显示 token 使用情况**：始终包含成本可见性的 token 分析
6. **测试真实行为**：验证实际创建的文件、测试通过、提交完成

## 会话记录格式

会话记录是 JSONL（JSON Lines）文件，其中每行是一个表示消息或工具结果的 JSON 对象。
