...
> **对于代理工作者：** 必需：使用 superpowers:subagent-driven-development（如果有子代理可用）或 superpowers:executing-plans 来实现此计划。步骤使用复选框（`- [ ]`）语法进行跟踪。

**目标：** 将视觉头脑风暴从阻塞式 TUI 反馈模型重构为非阻塞式"浏览器显示、终端命令"架构。

**架构：** 浏览器成为交互式显示器；终端保持对话通道。服务器将用户事件写入每个屏幕的 `.events` 文件，Claude 在其下一轮读取。不再需要 `wait-for-feedback.sh` 和所有 `TaskOutput` 阻塞。

**技术栈：** Node.js（Express、ws、chokidar）、纯 HTML/CSS/JS
...
| `lib/brainstorm-server/helper.js` | 修改 | 客户端 JS：移除 send/feedback 函数、缩小到点击捕获 + 指示器更新 |
| `lib/brainstorm-server/wait-for-feedback.sh` | 删除 | 不再需要 |
| `skills/brainstorming/visual-companion.md` | 修改 | 技能说明：重写循环为非阻塞流 |
...
在测试 4 区域之后添加到 `tests/brainstorm-server/server.test.js` — 一个新测试，发送带有 `choice` 字段的 WebSocket 事件并验证 `.events` 文件被写入：
...
删除 `window.send` 函数（第 120-129 行）— 绑定到已移除的发送按钮。
...
**注意：** 下面的行引用来自_原始_文件。任务 2 在文件前面插入了新测试，因此实际行号会偏移。按其 `console.log` 标签（例如"Test 5:"、"Test 6:"）查找测试。

- [ ] **步骤 1：更新测试 5（完整文档断言）**

找到测试 5 断言 `!fullRes.body.includes('feedback-footer')`。将其更改为：完整文档也不应该有指示器栏（它们按原样提供）：
...
- [ ] **步骤 4：用指示器栏测试替换测试 8（sendToClaude 主题）**
...
预期引用：只有 `visual-companion.md`（在任务 6 中重写）和可能的发布说明（历史记录，保持原样）。
...
移除关于"JS API"的 helper.js 引用描述——API 现在是最小的。保留路径引用：
...
预期：在发布说明和 spec/plan 文档之外没有匹配（它们是历史记录）。

- [ ] **步骤 4：如需清理则最终提交**

```bash
git status
# 审查未跟踪/修改的文件，根据需要暂存特定文件，如有需要则提交
```