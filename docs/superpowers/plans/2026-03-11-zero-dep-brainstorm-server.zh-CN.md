...
**目标：** 用单个零依赖的 `server.js` 替换头脑风暴服务器的 vendored node_modules，使用 Node 内置模块。

**架构：** 单个文件，带有 WebSocket 协议（RFC 6455 文本帧）、HTTP 服务器（`http` 模块）和文件监视（`fs.watch`）。在需要作为模块时导出协议函数以进行单元测试。
...
- **无变更：** `skills/brainstorming/scripts/helper.js`、`skills/brainstorming/scripts/frame-template.html`、`skills/brainstorming/scripts/stop-server.sh`
...
- [ ] **步骤 1：创建带有 OPCODES 常量和 computeAcceptKey 的 server.js**
...
客户端帧总是带掩码。返回 `{ opcode, payload, bytesConsumed }` 或不完整时返回 `null`。对未掩码帧抛出错误。
...
- [ ] **步骤 1：在 server.js 顶部添加配置和常量（在 requires 之后）**
...
- [ ] **步骤 2：在模块范围添加 WAITING_PAGE、模板加载和辅助函数**

在模块范围加载 `frameTemplate` 和 `helperInjection`，以便 `wrapInFrame` 和 `handleRequest` 可以访问。它们只从 `__dirname`（脚本目录）读取文件，无论模块是被 require 还是直接运行都有效。
...
文件监视逻辑内联在 `startServer`（步骤 6）中，以保持监视器生命周期与服务器生命周期在一起，并按规范为每个监听器包含一个 `error` 处理器。

- [ ] **步骤 6：添加 startServer 函数和条件 main**

`frameTemplate` 和 `helperInjection` 已在模块范围（步骤 2）。`startServer` 只创建屏幕目录、启动 HTTP 服务器、监视器，并记录启动信息。
...
测试目录已有一个 `package.json`，其中 `ws` 作为依赖。如有需要请安装，然后运行测试。
...
- [ ] **步骤 2：移除 gitignore 中的 node_modules 异常**
...
- [ ] **步骤 3：将 HTML 文件写入屏幕目录**

```bash
echo '<h2>Hello from smoke test</h2>' > /tmp/brainstorm-smoke/test.html
```

预期：浏览器重新加载并显示"Hello from smoke test"，包装在框架模板中

- [ ] **步骤 4：验证 WebSocket 工作 — 检查浏览器控制台**

打开浏览器开发工具。WebSocket 连接应显示为已连接（控制台中无错误）。框架模板的状态指示器应显示"Connected"。
