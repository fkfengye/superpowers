...
构建一个生成 ASCII 分形的 CLI 工具。参见 `design.md` 获取完整规格。
...
- 使用递归中点细分算法
- 创建带测试的 `internal/sierpinski/sierpinski_test.go`：
...
- 将迭代次数映射到字符渐变" .:-=+*#%@"（或如果提供单个字符则使用该字符）
- 创建带测试的 `internal/mandelbrot/mandelbrot_test.go`：
  - 输出尺寸与请求的 width/height 匹配
...
- 对于 Mandelbrot，`--char` 应使用单个字符而不是渐变
- 为自定义字符输出添加测试
...
**验证：**
- `./fractals sierpinski --size 0` 打印错误，退出非零
- `./fractals mandelbrot --width -1` 打印错误，退出非零
- 错误消息清晰且有帮助
